...
(async () => {
  ...
  await page.evaluateOnNewDocument(()=>{
    window.addEventListener('error', e=>{ console.log('WINERR:'+ (e.error&&e.error.stack ? e.error.stack : e.message)); });
  });
  page.on('console',m=>{ if(m.text().startsWith('WINERR')) console.log(m.text()); });
